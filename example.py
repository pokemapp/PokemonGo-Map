...
def get_args():
    ...
    parser.add_argument('-f', '--firebasetoken', help='Firebase token', required=False)
        ...
        try:
            for cell in hh.cells:
                for wild in cell.WildPokemon:
                    ...
                    print 'Wild: {}, {}'.format(pokemonsJSON[str(wild.pokemon.PokemonId)], hash)
...
transform_from_wgs_to_gcj(Location(Fort.Latitude, Fort.Longitude))
    ...
    print 'Visible: {}'.format(len(visible))
    for poke in visible:
        ...
        pokemons[poke.SpawnPointId] = pokemon
...
def push(poke):
    if args.firebasetoken != None:
        payload = {'to': args.firebasetoken, 'data': {'pokeId': poke['id'], 'pokename': poke['name'], 'lat': poke['lat'], 'lon': poke['lng'], 'hiddens': poke['disappear_time']}}        
        print("Pushing " + poke['name'])
